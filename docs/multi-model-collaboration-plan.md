...
1. **直接说需求（推荐）** — 你只要把目标告诉 Hermes，它会按下面的路由表自动判断该不该走多模型、该调谁，然后自己执行，无需你额外说明。
2. **显式口令（强制走多模型）** — 想明确进入协作流程时，用开头：
   - `多模型协作：<任务>`
   - `协作模式：<任务>`
3. **简单任务不惊动其他模型** — 只需 Hermes 自己处理（改个字符串、查定义、纯文本回答等，不动仓库文件）的任务，直接说即可，省钱省时间。

### 给 Hermes（跨会话自动触发）

- 这套流程已固化为 skill **`multi-model-collaboration`**（Hermes 会在相关场景自动加载），并存有持久记忆（触发词、文档路径、单一写者、worktree 隔离、kimi PATH）。
- 因此**任何会话**里，只要任务符合下表，Hermes 会自动按"路由 → 调用 → 验证 → 审阅 → 合并 → 汇总"执行，无需你每次提醒或粘贴文档。

### 路由速查（用户说什么 → Hermes 交给谁）

| 你说 | Hermes 处理 |
|---|---|
| "实现 X / 修复 Y / 跑构建"（会写仓库文件） | **MiMo**：worktree 独立分支实现 + `xcodebuild` 验证 → 审阅合并回主分支 |
| "出架构方案 / 评审设计 / 写技术文档" | **Kimi**：只读设计/文档评审 |
| "设计 + 实现"（混合） | **Kimi 出方案 → MiMo 实现** → 审阅合并 |
| "评审最近改动" | **并行**：Kimi 架构审查 + MiMo 实现审查（此阶段均只读）→ 汇总 |
| 简单查询/文本、不写文件 | **Hermes 自己** |

> **注意**：凡涉及写仓库文件的任务，Hermes 默认先在**独立 git worktree/分支**上执行，审阅通过后才合并回主分支——主目录在你确认前不会被改动。
