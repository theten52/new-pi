...
> **2026-09-12 A 文档工作台**：用户已选 A，生产阅读面与共用输入区已调整；单文档架构不变，原型仅作设计对照。实现范围、probe PARTIAL 与待验收项见 [实施记录](./dev-notes/2026-09-12-document-workbench-ui.md)。下方收尾修复摘要及历史调研仍按原时间范围理解，不追溯改写。
...
> **2026-09-11 加固状态**：没有重做单文档架构。冻结前缀的旧尾块边界误判已修复；
> 同值发布、预热暂停空转、隐藏文档投递、内容进程快照恢复、预算重复估算、
> Session 热恢复和聊天室原始 delta 消费路径已分别加固。
> JS 桥接现在单飞并仅暂存最新显示快照；全历史遍历及完整活动 body 编码仍然存在。
> 详见 [长会话性能复核 §11](./dev-notes/2026-09-11-long-session-rendering-review.md#11-修复实施与验证记录)，
> 其中明确区分已验证机制与尚未完成的真实 App 端到端性能归因。
> 随后真实 Session 的 200 行复现已抓到 RenderBox/CA 表面分配同步等待，仍会造成秒级 MainActor 延迟；
> 单飞 JS 投递并未解决这层问题，见同文 §12 的现场采样，不能把 §11 的回归通过当作卡顿已根治。
> 同日 §13 已用真实状态栏 A/B 复现并修复其中的持续 SF Symbol pulse 触发路径：
> 移除图标动画、保留文字呼吸，修复后采样无该等待栈；三轮含礼花回归最大延迟 46/9/16ms。
> 另补单围栏 Text 节点追加与协议尾段时间点；仍需在用户完整 Debug 场景复测，不更改单文档架构。
> 同日 §15 撤除流式 160px 高度量化，改用自然高度 + 文档统一 32px 尾距，
> 修复正文/工具切换释放占位造成的额外跳动。真实宿主三轮最大延迟 47.7/7.9/8.3ms，
> 交替输出与折叠尾距、上翻锚点回归通过；CV、绘制唤醒及唯一 Scroll writer 均保留。
> **残余问题（同日 §16，待修复）**：带空行列表在流式拆块与最终全文解析之间存在结构差异；
> 三项有序列表受控测试最终增高 26px，并由收尾钉底体现为 26px 位移。此前纯文本/单围栏回归未覆盖。
>
