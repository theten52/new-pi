...
以下项目前**未实现**，作为后续优化备忘；每一项写明现状、问题、建议，避免重复评估。
...
> **评审警示**：后续审查"每条消息一个 WebView / 会话切换"相关结论时，务必以真实代码 + 本文档（尤其 §3.10 的历史反证、本节待议项）交叉验证，勿仅凭单一 LLM 审查的"改一行"建议就动手——例如 `LazyVStack → VStack` 就是被判定为"前提属实、建议错误"的反例。
