...
> **当前状态（2026-09-12 源码核对）：历史方案，不能按原步骤继续实施。**
> 原生「每消息 WKWebView / 高度缓存 / 预热 / 滚轮转发」已被单文档 transcript 取代，
> 当前每个会话面板使用一个 `NewPiTranscriptDocumentView`，布局与滚动由 JS 自持。
> **已实现**：`NewPiViewModel.keptAliveRuntimes` + `NewPiChatView` 保活热 runtime / 面板；
> 冷恢复通过 `ScrollPositionStore` → `restoreEntry` → 首批 `restoreAnchor` 恢复锚点。
> 缓存目标为总 runtime 数 5；活跃、流式及待审批 runtime 不淘汰，因此并非绝对上限。
> 隐藏面板保留文档，但 Coordinator 只保存最新待应用 transcript 状态，恢复可见后再 flush；
> 不再沿用下文「后台各 WebView 持续刷新」的旧策略。这种内存状态快照也不是位图兜底。
> **未实现**：下文 `takeSnapshot` 位图兜底；也未接入历史 HTML 产物缓存重放
> （见 [渲染产物重放方案](rendered-result-replay-plan.md)）。代码中的旧快照注释不构成实现证据。
> 冷路径仍需重建文档并渲染，不能承诺任意会话都「零解析秒开」。本次未运行构建、测试或体验验收。
...
## 二、根因分析（旧 per-message 架构的历史核对）

> 以下描述与文件路径/符号保留为历史证据；不代表这些类或调用链仍在生产路径中。
...
以下保留旧清单，并在每项前标注当前处置；旧「现状」均指当时，不能直接当成当前缺陷。

### 1. 高度缓存对宽度不敏感（低危）
- **当前处置：已失效**。`MarkdownRenderingCache` 已移除，当前原生不维护每消息高度；不再补宽度 key。
- **现状**：`MarkdownRenderingCache` 以消息内容哈希为 key，**不含宽度**。
- **问题**：高度是宽度的函数。窗口 resize 后缓存值过期，冷重建会话时首帧可能用错高度（会被 JS 回调纠正，但会闪一下）。
- **建议**：把面板宽度纳入 key，或监听宽度变化清空缓存。收益小、复杂度中等，**暂缓**（接受"偶发冷重建首帧闪一下"）。

### 2. `keptAliveRuntimes` 依赖隐式刷新（中危，脆弱）
- **当前处置：保留为待评估的可维护性问题**。计算属性仍存在，但旧 `reflectActive()` 路径已变化；
   本次未验证有漏刷新故障，不能据历史推理直接判定当前缺陷。
- **现状**：`runtimes` 非 `@Published`，`keptAliveRuntimes` 是计算属性——靠"其它 `@Published` 恰好都会变"来触发面板列表重算。
- **问题**：目前所有增删路径都伴随 `reflectActive()`（会改 `@Published`），所以能工作；但**脆弱**——以后若加一条不碰 `@Published` 的 runtime 变更路径，面板列表会静默不更新。
- **建议**：把保活列表做成**显式维护的 `@Published`**；或在 `evictIdleRuntimesIfNeeded` / `beginSession` 增删 runtime 后显式刷新。**倾向先记录，新增 runtime 变更路径时再加固。**

### 3. 被淘汰会话切回无滚动恢复（建议下一步做）
- **当前处置：锚点恢复已实现**。`NewPiChatView.swift` 传入保存锚点，
   `NewPiTranscriptDocumentView.swift` 首批下发 `restoreAnchor`；位图快照兜底仍未实现，二者不可混同。
- **现状**：LRU 淘汰的会话（第 6+ 个）切回走冷重建，滚动**落到底部**（不恢复原位置）。
- **问题**：违背"原位恢复"目标；影响去翻很早会话的场景。
- **建议**：先补**轻量"锚点 ID + 行内偏移"恢复**——切走时记录当前可见首条消息 id + 偏移，冷重建后 `scrollTo` 该锚点。快照（方案三）是更重的兜底，先不急。

### 4. 单元测试（可选）
- **当前处置：旧缓存/滚轮转发测试对象已失效**。若补测试，应围绕现有 runtime 淘汰和文档锚点恢复设计；本次未核定覆盖率或运行测试。
- **可测纯逻辑**：`MarkdownRenderingCache` 的存取/淘汰、`keptAliveRuntimes` 的排序/淘汰、滚轮转发状态同步。
- **前提**：确认测试 target 是否涵盖这些逻辑（`NewPiCore` 或 app target 的可测部分）。

---

> **评审警示**：后续审查会话切换时，先核对当前单文档源码，再参考本文历史。
> 原警示提到的「§3.10」并非本文章节，不作为有效定位；`LazyVStack → VStack`
> 曾被判为「前提属实、建议错误」的历史反例，也不能据此恢复旧 per-message 架构。
