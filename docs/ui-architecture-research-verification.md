...
> **2026-09-12 A 文档工作台**：用户已选 A，生产阅读面与共用输入区已调整；单文档架构不变，原型仅作设计对照。实现范围、probe PARTIAL 与待验收项见 [实施记录](./dev-notes/2026-09-12-document-workbench-ui.md)。下方收尾修复摘要及历史核验仍按原时间范围理解，不追溯改写。
...
>
> **后续状态（2026-08-30）**：本核验驱动的单文档迁移已完成并删除遗留路径。
> 本文引用的 NewPi 文件/行号均为迁移前快照，仅作历史记录；
> 现行架构见 [`ui-architecture-decision.md`](./ui-architecture-decision.md) §4.3。
> 当前源码入口与历史章节定位统一见 [ADR §1](./ui-architecture-decision.md#1-结论)，
> 文档导航见 [`README.md`](./README.md)。本文 §0–§5 的“当前／已完成／待办”均为原核验快照，
> 不应将原生高度表、preheater、`engineFingerprint` 或 NO-GO 退路重新列入当前 backlog。
> **JS Warmer / Poller 仍是现行文档内机制**，不在原生遗留路径的删除范围内。
>
> **后续核验（2026-09-11）**：下文“已实现冻结前缀”不代表实现无缺陷；
> 正常旧尾块完成时误重建前缀的问题已复现并修复，高亮提升与内部节点身份新增真实 WKWebView 回归。
> 同批加入发布去重、预热暂停、隐藏文档/单飞快照、恢复、预算缓存和聊天室后台消费加固；
> 当前证据与保留限制见 [长会话性能复核 §11](./dev-notes/2026-09-11-long-session-rendering-review.md#11-修复实施与验证记录)。
> **同日现场复现补充**：200 行输出时已采到 RenderBox/CA 同步等待栈；原生呈现卡顿仍未根治，
> 详见该文 §12。JS 执行时间不等于最终呈现时间。
> **同日修复验证（§13）**：只有加入真实状态栏才复现；持续 SF Symbol pulse 是已验证触发点。
> 移除图标动画并保留文字呼吸，20s 采样不再出现该等待栈；含礼花回归最大延迟 46/9/16ms。
> 单围栏追加另改为稳定 Text 节点，200 行追加期间子树重建为 0；不把这项优化当作秒级阻塞根因。
> **底部留白修复（同日 §15）**：撤除流式 160px 高度量化，正文/工具均自然高度，
> 文档统一 32px 尾距并正确处理隐藏的尾部详情。几何、保锚与真实宿主回放通过，
> 三轮最大 MainActor 延迟 47.7/7.9/8.3ms；未删除 CV、绘制唤醒或改写滚动状态机。
> **残余核验（同日 §16，未修复）**：空行分隔列表在流式期分拆、最终态合并为松散列表；
> 相同 source 的三项有序列表最终文章增高 26px，随后收尾滚动位移 26px。未认定用户原始输出必为同一原因。
>
...
> **历史核验结论**：以下三项缺口是在迁移前成立，不是本次对当前源码重新判定的缺口。
...
| 报告提出的真实缺口 | 迁移前三项成立（见 §4）；现已落地或退役，当前处置见 §5 |
...
> **旧实现取证**：以下缓存与原生 preheater 均已退役；保留以解释当时核验修正，不是当前缓存架构。
...
> **迁移前快照**：§3.1–§3.2 的核心渲染能力延续为 per-root 实例；§3.3 的原生 rail 实现及
> §3.4 的高度桥/预热/窗口化/persistent replay 已退出生产路径。旧行号与代码片段只作历史证据。
...
> 此处“已实现”指旧原生路径；当前为文档内 `Scroll.jumpTo` 与 JS 实测比例驱动的 rail。
...
> **历史缺口归档，非当前待办**：§4.1–§4.2 的原生高度缓存/高度表已退役；
> §4.3 的滚动意图纪律已在文档内落地。以下方案、优先级与“目前”均只描述迁移前状态。

去掉当时已完成项后，原报告中成立的缺口是以下三项。

### 4.1 高度缓存 key 未纳入引擎指纹 —— 有实际隐患

> **当前处置：旧缓存与 `engineFingerprint` 已退役，不再接入高度查询。**
...
> **当前处置：原生高度表已删除，无需下沉或补 blockID 高度上报。**
...
> **当前处置：文档内 Scroll 意图纪律已落地；Poller 在允许的意图下作几何补偿，不恢复原生 writer。**
...
| 原建议 | 当前状态 |
|---|---|
| 块级增量 / 冻结前缀 / 尾部规范化 | 保留；后续缺陷与回归边界见文首加固记录，不重复实现 |
| 显式滚动状态机 | 已由文档内 Scroll 意图纪律承接 |
| engineFingerprint 高度 key / block 级原生高度表 | 随旧机制退役，不属当前 backlog，也不是默认 fallback |
| 原生 rail pending jump / 高度桥 / preheater / 窗口化 / 持久 HTML replay | 旧实现退役；当前为文档内布局、条目锚点恢复和最新快照重建 |
| 显式 RenderBlockID | 原可选设计，不能据旧清单直接排期，需针对当前问题另行论证 |

文档内 Warmer / Poller 不等于旧原生预热器；现行宿主关闭逐消息 `reportHeight` / `postSnapshot`，
JS 中保留兼容 API 不代表恢复了旧路径。单文档为跨消息选择与全文查找提供基础条件，
本记录不据此宣称交互已完整验收。

以下是**迁移前排期快照，仅保留取证，不再执行**：
