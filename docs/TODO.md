...
- **已验证候选**：当前 `splitBlocks` 按空行切分，可能把同一个松散列表拆成多个独立列表；
...
- **后续方向**：先对齐流式/最终的 Markdown 块语义（尤其列表、引用、跨块引用链接），
  再检查 CV 估高交接与收尾滚动；不删除最终正确解析、不恢复 160px 占位、不用动画掩盖。
- **验收**：同源最后流式快照与最终态的结构/高度对比、末行屏幕位置、32px 尾距、
  上翻保锚及 200 行呈现性能；应补入含空行列表/嵌套列表，不能只依赖纯文本和单围栏测试。
- **详细证据与最小复现**：[性能复核 §16](dev-notes/2026-09-11-long-session-rendering-review.md#16-非围栏-markdown-结束时约一行跳动待修复)。
...
| BACKLOG-COMPOSER-MULTILINE | 输入框多行（当前仅一行） | done | P1 | 已实现：`chatComposer` 的 `TextField` 换成 NSTextView 封装 `NewPiComposerTextView`（`NewPiChatView.swift`）：真实多行、固定 4 行高（超出后内部滚动）、Return 发送 / Shift+Return 换行（含 IME 组词保护）、保留 streaming 禁用；Send 按钮的 Return 快捷键已移除避免双重触发。 |
...
| BACKLOG-IMAGE-INPUT | 图片输入：支持选择、粘贴图片，且已添加的图片可移除 | done | P1 | 多模态输入能力，三个子需求：① **选择图片输入**——输入框提供添加图片入口（`NSOpenPanel` / SwiftUI `fileImporter`，过滤图片类型），选中后在输入框内/上方以缩略图形式展示；② **粘贴图片输入**——输入框支持 `Cmd+V` 从剪剪贴板粘贴图片（`NSPasteboard` 读取 `.tiff`/`.png` 等图片 data）；③ **移除已添加图片**——每张缩略图带「×」按钮，点击移除并更新待发送附件列表。发送时图片需作为多模态 content 附加到用户消息（`ModelTypes.swift` 消息构造需支持 image content），并贯穿 provider 序列化层（当前 DeepSeek/GLM 等 provider 的多模态支持需确认）。涉及 `NewPiChatView.swift`（composer + 缩略图区）、`NewPiViewModel.swift`（消息构造/发送）、`ModelTypes.swift`（多模态 content 类型）、provider 消息序列化。**Snipaste 粘贴修复（2026-08-31）**：Snipaste 等工具复制图片后 Cmd+V 无反应（beep），根因是 `NewPiComposerInnerTextView` 未重写 `validateUserInterfaceItem`，NSTextView 默认在 `isEditable=false`（streaming 时）或 responder chain 异常时返回 false 禁用粘贴命令。修复：重写 `validateUserInterfaceItem` 对 `paste:` 始终返回 true；同时扩展 `PasteboardImageReader` 支持的类型（新增 `public.jpeg`、`image/png`、`image/tiff`、`image/jpeg` 等 MIME 类型）。 |
