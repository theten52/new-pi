...
> ⚠️ **已被取代（2026-08-29）**：收敛校正机制（ChatScrollCoordinator + 行内 GeometryReader 上报 +
> deadline 窗口）已整体删除。新方案为「高度表 + 算术定位」：`NewPiTranscriptHeightMap` 为每行
> 提供确定性高度（md 行读渲染缓存[预热全量化]、You/文本行按字体×宽度纯函数计算、工具行折叠态固定高），
> rail 点击 = 前缀和算 y → `scrollTo(point:)` 一次到位，行实测只回写表格不触发二次滚动。
> 本文档保留作历史根因参考，勿再按此实现。
