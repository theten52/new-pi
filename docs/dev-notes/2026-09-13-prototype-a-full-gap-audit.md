...
## 当前结论：剩余 4 类，涉及 5 个部分完成项

以 **`e0de77a`** 为生产代码基线，2026-09-13 再次核对。统计范围仅为原型 A 的旧清单，不是整个项目 backlog；下方历史审计不能直接计入当前缺口。

| 剩余类别 | 旧清单 ID | 已有能力 | 尚未完成 | 当前源码依据 |
|---|---|---|---|---|
| 侧栏历史摘要 | A-16、A-17 | 已加载会话显示执行状态、相对时间及已记录编辑数量 | 未加载会话仍显示状态未知，没有完整历史状态/编辑摘要；已有创建时间的相对格式，不是完全没有时间展示 | `NewPiApp.swift`：`SessionRow`、`LiveSessionRow` |
| 聊天室下一位 | A-18 | 具名下一角色及真实推进按钮 | `nextSpeakerFooter` 仍是正文下方原生固定尾行，未随文档滚动 | `NewPiApp.swift`：`nextSpeakerFooter` |
| 聊天室历史轮次 | A-19 | 显式阶段分隔线 | 历史消息无轮次字段，无法展示准确历史轮数；不能用当前评审轮数回填 | `NewPiChatRoomTranscriptAdapter.swift`：阶段条目构造 |
| 保护范围结论 | A-08 | 图标化工具/编辑统计、真实 JUnit 报告 | 尚无自动证明“未修改滚动机制”等结论的可靠数据来源；测试通过不等于保护范围证明 | `transcript-document.js`：`syncAnswerFooters`、`testReportSegments` |

### 不计入剩余缺口

- **用户已撤销**：顶部「改动」、回答底部「查看改动」、审批「查看差异」及专用实现/测试；V-06、V-07 和相关按钮子项不再排期，原 Git UI 验收也随能力删除撤销。
- **已清理空提示**：「本轮未记录文件编辑快照」「未提供测试报告」不再显示；有实际记录/报告时仍显示真实统计。`update_plan` 自报计划和 `read_test_report` 读取能力保留。
- **已接入，不重复建设**：过程总/逐步图标、两行工具说明、单步耗时与停止态、计划 X/Y、复制图标/反馈、审批层级/回执、错误图标、输入焦点/提示、建议焦点交接、附件文件名、空态输入区、头像/主要正文排版、角色头像栏、⌘N/日期上下文、模型图标/发送禁用态、圆环旋转及窄宽分栏策略。
- 系统侧栏按钮保留用户选择；快照捕获与已有记录保持原有边界，不因旧原型再次扩展。

### 验证欠账单列，不算新增功能缺口

完整 App 同内容视觉对照、真实输入法、VoiceOver 及完整导航/焦点路径尚未全部验收。已有组件回归不能代替这些验收，也不能因此把已接入功能重新标为未实现。两条空提示最后调整的正文回归已通过，冷恢复结果未确认；此前移除 diff 后的通过/跳过及用量偶发失败记录见 `docs/TODO.md`。本次仅更新结论，不运行新测试、不改生产代码。
...
## 历史实施检查点（187b0fe，非当前待办）

> 本节保留 187b0fe 实施过程及当时的验证阻塞；其后的删除决定、构建结果和剩余数量以上方当前结论为准。不能把这里的“当前”“未提交”或 Git UI 阻塞当成最新状态。
