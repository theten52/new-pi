...
> **后续审计纠正**：本草稿是已交付子集，不代表 A 原型全部实现；错误卡/真实重试、diff、空态建议、
> 消息元数据与审批样式等仍有缺口，详见[六场景审计](2026-09-12-prototype-production-gap-audit.md)。
> 用户另报已发送图片预览比例错误；后续修复不包含在下文旧 Release 哈希中。
