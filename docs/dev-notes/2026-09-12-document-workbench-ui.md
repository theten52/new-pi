...
> **后续范围纠正**：阶段实施不等于原型全部交付。六场景实查确认错误卡/重试、真实 diff、空态建议、
> 消息元数据及部分状态摘要缺失，审批仍为模态而非内联；见[原型差距审计](2026-09-12-prototype-production-gap-audit.md)。
...
### 正式 App 非空草稿与菜单焦点（06:07 起）

`NativeSidebarChecks.swift` 增加显式 `composer-inspect` / `composer` 模式，源码在工作区内。
先确认当前普通会话输入为空、无附件且未运行，再经 AX 获得输入焦点，向目标 App PID 投递
固定 Unicode 文本的真实键盘事件。发送按钮随文本启用，证明不是只设置显示值的替代测试。

正式 App 实跑通过：非空测试草稿经系统侧栏收起/展开保持；模型菜单和用量弹窗分别打开、
Escape 关闭后自动恢复原输入框焦点，继续键入的文本追加到原草稿；模型与思考级别的 AX 值未改变。
最后用当前输入框的 ⌘A/Delete 清除精确匹配的测试文本，验证空输入、发送重新禁用；
随后原侧栏 `check` 和只读输入检查通过。未按 Return、未选择模型、未发送消息、未访问剪贴板。
首轮在写入前因系统菜单栏常驻子菜单被误判而中止；排除 AXMenuBar 后通过，未绕过实际弹出菜单检查。

清理只处理本次测试文本；如果用户同时改动输入或目标 App 失去前台，宁可失败并保留内容，
也不覆盖用户输入。测试不清除用户撤销历史，undo 栈可能含测试编辑。此次未发现生产缺陷，未改生产代码。
这不是真实 IME 候选确认、跨 Session/聊天室草稿、运行中输入或完整 VoiceOver 验收。

## 4. 尚需人工验收与结果回填

- [x] 后续发现并修复跨模式视图重建丢稿：运行时持有独立草稿，旧版目标断言失败、新版隔离回归与构建通过；见[草稿生命周期记录](2026-09-12-navigation-draft-lifetime.md)。不等同于正式整窗导航/滚动组合验收完成。
...
- [x] 正式 App 非空临时草稿经侧栏往返保持，模型菜单/用量弹层 Escape 后焦点恢复与续写通过，临时文本清理已确认。
...
- [ ] 实际附件选择/拖拽/粘贴/移除/预览、中文输入法确认与完整键盘路线（模型/用量 Escape 后的输入焦点已验证）。
